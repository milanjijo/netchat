...
int PosixNetworkConnection::acceptClient(int server_fd) {
    sockaddr_in client_addr;
    socklen_t addr_len = sizeof(client_addr);
    return accept(server_fd, (struct sockaddr*)&client_addr, &addr_len);
}

void PosixNetworkConnection::closeSocket(int sock) {
    close(sock);
}

int PosixNetworkConnection::connectToServer(const std::string& ip, int port) {
...
std::string PosixNetworkConnection::receiveMessage(int sock) {
