...
SelectStrategy::SelectStrategy(Server* server, size_t numWorkers)
    : running_(false), numWorkers_(numWorkers), serverSocket_(-1), 
      netManager_(nullptr), server_(server) {
}
...
void SelectStrategy::workerThread() {
    ...
    while (running_) {
        ...
        bool shouldContinue = server_->processClientMessage(item.clientSocket, item.userID, message);
